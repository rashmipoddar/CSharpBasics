...
namespace CSharpBasics
{
    public class StringBasics
    {
        ...
        public static void run()
        {
            ...
            modifyString(greeting); // ello World
            removeFirstAndLastCharFromString(greeting); // ello World
            getCharsAtSpecificIndex(animal); // ehs
            isCharInString(greeting, h); // True
            getIndexOfCharInString(greeting, h); // 0
            getFileNameAndDrive(filePath); // bar \n C
            splitStringToArray(greeting); // Hello \n World!
            formatStringWithFormattingTokens(message); // My name is Sara. I live in Seattle.
        }
